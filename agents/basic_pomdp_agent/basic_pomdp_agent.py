...
class BasicPOMDPAgent:
    ...
    def choose_action(self) -> Action:
        ...
        # Update belief based on new observations since last action
    ...
    def _should_knock(self):
        """Decides whether to knock based on expected hand value."""
    ...
    def _best_peek_action(self, actions) -> Action:
        ...
        # Look for the opponent slot with highest expected value that we don't already know
        for action in actions:
            if action.action_type != ActionType.PEEK:
                continue # only consider peek actions
            if action.target_player != opponent_idx:
                continue # only consider peeks targeting opponent's hand
    ...
    def _best_keep_action(self, actions) -> Action | None:
        ...
        best_action = None
        best_ev = current_ev  # only keep if it strictly improves expected value
    ...
    def _best_swap_action(self, actions) -> Action:
        ...
        Positive delta means it's moving high value out and bringing low value in.
    ...
    def _best_kingpin_action(self, actions) -> Action:
        ...
        # Priority: eliminate known RAT first (can only be removed by Kingpin)
        for action in actions:
    ...
    def _best_draw_action(self, actions):
        ...
        if ActionType.DRAW_FROM_DISCARD in [a.action_type for a in actions]:
            ...
            if top and top.value is not None:
                ...
                # Calculate expected values for all known slots in hand to find the worst one
                valid_evs = [
