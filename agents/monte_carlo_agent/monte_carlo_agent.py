...
class MonteCarloAgent:
    ...
    def _rollout_policy(self, sim, actions):
        ...
        # DRAW: prefer discard if top card is low value numbered card, else draw from pile
        if ActionType.DRAW_FROM_DISCARD in action_types or ActionType.DRAW_FROM_PILE in action_types:
            ...
            if top and top.value is not None and top.value < 6:
        ...
        # PEEK: peek at first available target (rollout doesn't need to be smart here)
        ...
        # Fallback — should never hit this if all phases are covered
        return actions[0]