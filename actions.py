...
class Action:
    ...
    def _execute_draw_from_discard(self, game, GamePhase, agent=False):
        ...
        if game.state.is_user_turn() and hasattr(game.agent, 'observe'):
            game.agent.observe(Observation(ObsType.PLAYER_DRAW_DISCARD,
                                           card=card,
    ...
    def _execute_discard_drawn(self, game):
    ...
    def _execute_swap(self, game):
        ...
        if hasattr(game.agent, 'observe'):
            ...
            game.agent.observe(Observation(obs_type,
                                           p1=player1_idx, s1=card1_idx,
                                           p2=player2_idx, s2=card2_idx))
    ...
    def _execute_kingpin_eliminate(self, game):
    ...
    def _execute_kingpin_add(self, game):
    ...
    def _execute_peek(self, game, GamePhase, agent=False):
        ...
        if game.state.is_agent_turn() and hasattr(game.agent, 'observe'):
            ...
            game.agent.observe(Observation(ObsType.AGENT_PEEK,
                                           card=peeked,
